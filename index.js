  ...
  function getBuyableItem(coin) {
    if (coin < 500) return '사용가능한 음료수 : 없음';
    let result = [];
    ...
    return `사용가능한 음료수 목록 => ${result.join(', ')}`;
  }

  // 음료를 고르는 함수
  function selectedItem(item) {
    if (beverageData[item].stock <= 0) return `${item}는 재고가 없습니다.`;
    if (beverageData[item].price > accumCoin) return `금액이 모자랍니다.`;
